  ...
  def of[Path, Element](
    ...
    object MatchesAndTheirSections:
      ...
      lazy val empty = MatchesAndTheirSections(
        ...
        baseSpanningSectionsByPath = Map.empty,
        leftSpanningSectionsByPath = Map.empty,
        rightSpanningSectionsByPath = Map.empty
      )
      ...
      private def subsumesSection(
          ...
          spanningSectionsByPath: Map[Path, Section[Element]]
      )(section: Section[Element]): Boolean =
        val path = side.pathFor(section)

        spanningSectionsByPath
          .get(path)
          .collect {
            case spanningSection if spanningSection != section => true
          }
          .getOrElse(
            sectionsByPath
              .get(path)
      ...
      private def subsumesSectionViaAtLeastOneAllSidesMatch(
        ...
        spanningSectionsByPath
          .get(path)
          .collect {
            case spanningSection if spanningSection != section =>
              sectionsAndTheirMatches.get(spanningSection).head
          }
    ...
    case class MatchesAndTheirSections(
        ...
        sectionsAndTheirMatches: MatchedSections,
        baseSpanningSectionsByPath: Map[Path, Section[Element]],
        leftSpanningSectionsByPath: Map[Path, Section[Element]],
        rightSpanningSectionsByPath: Map[Path, Section[Element]]
    ):
      import MatchesAndTheirSections.*

      private val subsumesBaseSection: Section[Element] => Boolean =
        subsumesSection(sectionsAndTheirMatches)(
          base,
          baseSectionsByPath,
          baseSpanningSectionsByPath
        )
      private val subsumesLeftSection: Section[Element] => Boolean =
        subsumesSection(sectionsAndTheirMatches)(
          left,
          leftSectionsByPath,
          leftSpanningSectionsByPath
        )
      private val subsumesRightSection: Section[Element] => Boolean =
        subsumesSection(sectionsAndTheirMatches)(
          right,
          rightSectionsByPath,
          rightSpanningSectionsByPath
        )
        ...
        matches.foldLeft(this) {
          case (
            ...
            matchesAndTheirSections
              ...
              .focus(_.baseSpanningSectionsByPath)
              .modify(
                _ -- matchesAndTheirSections.baseSpanningSectionsByPath
                  .get(basePath)
                  .collect {
                    case spanningSection if baseSection == spanningSection =>
                      basePath
                  }
              )
              ...
              .focus(_.leftSpanningSectionsByPath)
              .modify(
                _ -- matchesAndTheirSections.leftSpanningSectionsByPath
                  .get(leftPath)
                  .collect {
                    case spanningSection if leftSection == spanningSection =>
                      leftPath
                  }
              )
              ...
              .focus(_.rightSpanningSectionsByPath)
              .modify(
                _ -- matchesAndTheirSections.rightSpanningSectionsByPath
                  .get(rightPath)
                  .collect {
                    case spanningSection if rightSection == spanningSection =>
                      rightPath
                  }
              )
        ...
        aMatch match
          case Match.AllSides(baseSection, leftSection, rightSection) =>
            copy(
              ...
              baseSpanningSectionsByPath = baseSpanningSectionsByPath ++ base
                .pathWhenSectionSpansFile(
                  baseSection
                )
                .map(_ -> baseSection),
              leftSpanningSectionsByPath = leftSpanningSectionsByPath ++ left
                .pathWhenSectionSpansFile(
                  leftSection
                )
                .map(_ -> leftSection),
              rightSpanningSectionsByPath = rightSpanningSectionsByPath ++ right
                .pathWhenSectionSpansFile(
                  rightSection
                )
                .map(_ -> rightSection)
